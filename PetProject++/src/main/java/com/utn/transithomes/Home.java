...
public class Home {
    ...
    public boolean cumpleRequisitosDelHogar(FormLostPet formularioMascotaPerdida){
        return (aceptaEspecie(formularioMascotaPerdida.getEspecie()) &&                                      //Especie
                ...
                cercaDelLugarEncontrado(formularioMascotaPerdida.getLat(),formularioMascotaPerdida.getLong()) &&                    //Lugar Encontrado
                cumpleCaracteristicas(formularioMascotaPerdida.getCaracteristicasDeLaPublicacionDelHogar()));        //Otras Caracteristicas


    }

    private boolean aceptaEspecie(String especie){              //Especie
        if(especie == "perro"){
    ...
    private boolean cercaDelLugarEncontrado(double latitud, double longitud){         //Lugar Encontrado
        Double distanciaMaximaEnKM = 15.0; //TODO VER EN QUE LUGAR SE PUEDE PEDIR ESTA VARIABLE. POSIBILIDAD 1 AL LLENAR EL FORMULARIO =)

        return (distanciaCoord(this.ubicacion.getLat(), this.ubicacion.getLong(), latitud, longitud) <= distanciaMaximaEnKM);
    }
...
// GETTERS AND SETTERS
    ...
    public void setLugaresDisponibles(int lugaresDisponibles) {
        this.lugares_disponibles = lugaresDisponibles;
    }
